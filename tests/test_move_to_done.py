...
class TestMoveToDone:
    def test(self, constants, logging_setup, output_directory):
        ...
        MoveToDoneTester().main(constants, configuration_file, output_directory)


# ----------------------------------------------------------------------------------------
class MoveToDoneTester(BaseContextTester):
    """
    ...
    async def __run_the_csv_test(self, constants, output_directory):
        ...
        filename = "Batch-1_9bg5_2023-05-04_RI1000-0080-3drop.csv"

        # Write csv.
        await dataface.write_csv(visitid, data, f"echo/{filename}")

        # Move to done.
        transfer_type = "soak"
        transfer_subdirectory = "echo"
        await dataface.move_to_done(visitid, filename, transfer_type)

        # Read csv from the done.
        fieldnames = [0, 1]
        restval = "none"
